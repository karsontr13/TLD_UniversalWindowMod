...

{
...

    {
        ...
        private Wind wind; // YENİ: Rüzgar bileşenini ekledik
        ...
        // YENİ: Şu an oynatılan videonun tam yolunu takip ediyoruz (Gereksiz geçişleri önlemek için)
...

        {
            ...
            wind = GameManager.GetWindComponent(); // Rüzgarı oyundan çekiyoruz
        ...
        // YENİ: Rüzgarın şiddetine göre dosya ismine eklenecek takıyı belirleyen metot
...

        {
            ...
            // Rüzgar hızını MPH (Mil/Saat) cinsinden alıyoruz

            // Not: TLD sürümlerine göre GetSpeedMPH() yerine GetWindSpeedMPH() olabilir, hata verirse düzeltirsin.

            float mph = wind.GetSpeedMPH();



            if (mph >= 25f) return "_VeryWindy"; // Şiddetli fırtına

            if (mph >= 12f) return "_Windy";     // Normal rüzgar



            return ""; // Sakin hava (Calm)

        }



        // GÜNCELLENDİ: ClearAurora için isimlendirme toleransı eklendi
...

        {
            ...
            // EĞER HAVA AURORA İSE (Kayıt sistemi ismine _Night eklemiş olabilir veya sade bırakmış olabilir, hepsini tara)
...

            {
                ...
                suffixesToTry.Add(""); // Standart

            }

            // DİĞER HAVA DURUMLARI İÇİN GECE KONTROLÜ
            ...
            // Listeyi sırayla kontrol eder. Rüzgarlı video yoksa otomatik normale geçer.
...

        {
...

            {
...

                {
                    ...
                    // Hedef videonun ne olması GEREKTİĞİNİ soruyoruz

                    string targetVideoPath = ResolveVideoPath(gameWeather, isNightTime, currentWindSuffix, out bool targetWillBeNight);



                    // EĞER hedef video, şu an oynatılan videodan farklıysa geçişi tetikle (Rüzgar değiştiyse ve videosu varsa tetikler)
...

        {
            ...
            // Eğer oynatılan video ile hedeflenen video tamamen aynıysa hiç işlem yapma, kaynak tüketme
        ...
        // GÜNCELLENDİ: Artık doğrudan hedeflenen video yolunu parametre alıyor

        private void TriggerWeatherChange(string newWeather, string targetVideoPath, bool targetNightVariant)
        {
            // DÜZELTME: Yumuşak geçiş devam ederken yeni bir tetikleme gelirse, ani güncelleme yapmak yerine 
            // mevcut geçişin tamamlanmasını bekliyoruz. Böylece ekran aniden kesilmiyor.
            if (isTransitioning || isWaitingToPrepare)
            {
                return;
            }
...

        {
            ...
            currentPlayingVideoPath = NextVp.url; // Yeni video oynatılıyor, kaydet!
...

        {
            ...
            currentPlayingVideoPath = absoluteVideoPath; // Şu an oynatılan yolu kaydet
...

    {
        ...
        public override void OnSceneWasInitialized(int buildIndex, string sceneName)
        {
            ...
            // 3. Önce GUID klasörüne bak, özel çekim yoksa Base (Ana) klasöre düş
            ...
            if (Directory.Exists(currentSceneFolder))
            {
                // Her iki ismi de gönderiyoruz ki konumlandırmalar bozulmasın
        ...
        private void ReplaceWindowsInScene(string sceneFolderPath, string baseSceneName, string uniqueSceneId)
        {
            ...
            foreach (var window in sortedWindows)
            {
                ...
                // Fonksiyona uniqueSceneId de ekledik
        ...
        private void ModifySingleWindow(GameObject windowObject, int index, string sceneFolderPath, string baseSceneName, string uniqueSceneId)
        {
            ...
            // HEM GUID hem de Base isim için kontrol anahtarları oluşturuyoruz
            string overrideKeyGuid = $"{uniqueSceneId}_{index}";
            string overrideKeyBase = $"{baseSceneName}_{index}";

            // ROTASYON KONTROLÜ (Tuzakçı Barakası gibi özel ayarların bozulmaması için)
            ...
            // Boyutlandırma işlemleri (Aynı şekilde kalıyor)
            float finalX = 1.0f;
            float finalY = 1.0f;
